...
var (
	_ api.DetectorPlugin          = &PendingChecker{}
	_ api.AlgorithmPlugin         = &PendingChecker{}
	_ api.PolicyStoreSyncerPlugin = &PendingChecker{}
)
...
func New(plArgs apiruntime.Object, rfh framework.ReschedulerFrameworkHandle) (godel_framework.Plugin, error) {
	...
	if args, ok = plArgs.(*config.PendingCheckerArgs); !ok || args == nil {
...
func (p *PendingChecker) Name() string {
	return Name
}

func (p *PendingChecker) Detect(_ context.Context) (api.DetectorResult, error) {
	return api.DetectorResult{}, nil
}

func (p *PendingChecker) SyncPolicyStore(ctx context.Context, snapshot *reschedulercache.ReschedulingSnapshot) policies.PolicyStore {
	return newPendingCheckerStore(snapshot.GetPendingPods(), p.args)
}

func (p *PendingChecker) Reschedule(ctx context.Context, detectRes api.DetectorResult) (api.RescheduleResult, error) {
